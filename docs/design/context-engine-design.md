...
**graph 連携は compose で行う。** seed-based な関連ノート収集は `excli-traverse:reach` でパスリストを取得し、その結果を `excli-read:bulk path=...` に渡すことで実現する。`read:bulk` 自体はグラフ traversal を持たない。
...
```
obsidian excli-read:bulk path=<path> [path=<path> ...] [max-files=<n>] [max-tokens=<n>] [include-frontmatter] [resolve-embeds] [embed-depth=<n>] [annotate-embeds] [format=<markdown|json|tsv>]
obsidian excli-read:bulk [folder=<path>] [tag=<tag>] [sort=<path|mtime|size>] [max-files=<n>] [max-tokens=<n>] [include-frontmatter] [resolve-embeds] [embed-depth=<n>] [annotate-embeds] [format=<markdown|json|tsv>]
```
...
| option | 型                           | 説明                                           |
| ------ | ---------------------------- | ---------------------------------------------- |
| `path` | vault-relative markdown path | `.md` を省略しない canonical path を基本とする |

複数 path は `path=<path>` の repeatable option とする（comma-separated を使わない理由: path 自体に comma を含む場合に壊れやすく、既存 command の option design とも不一致になる）。CLI adapter が repeated flag を潰す場合のみ JSON array fallback を許容する。
...
全 command 共通の概算 token rule:

- 推定式: `estimatedTokens = ceil(charCount / 4)`
- `charCount` には返却する本文・frontmatter・annotation comment を含める
...
| `max-tokens` が正の整数でない                                    | usage error     |
...
````markdown
<!-- excli-read:bulk count=2 truncated=false -->
...
```tsv
path	relation	estimated_tokens	truncated
papers/foo.md	explicit	312	false
papers/bar.md	explicit	287	false
```
...
4. budget 適用    → token budget と max-files で deterministic に打ち切る
...
- `path` repeatable option と JSON array fallback
- folder / tag フィルタリング
...
pnpm run test
pnpm run build
./bin/obsidian-dev obsidian excli-read:bulk path=...
