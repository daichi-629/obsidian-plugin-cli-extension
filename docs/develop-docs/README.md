# excli development docs

The previous root `README.md` content is preserved here for repository and development workflow reference.
...
The development vault is mounted at `/config/vault` inside the container and is backed by `./vault`
in this repository.

Treat `scripts/generate-test-vault.mjs` as the source for the test vault's baseline data, and use
`pnpm run vault:generate` when you want to reset or refresh that starting state. Tests may still edit
the vault manually or through commands after generation.

The default exposed ports are `3000` and `3001`. To change them, edit the `ports` section in `compose.yml`.
...
Tracked benchmark definitions live in `perf/`. Runtime prompts and results are written to `perf/prompts/`
and `perf/results/`.
...
The development container defined in `Dockerfile.obsidian`
