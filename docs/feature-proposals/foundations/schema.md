...
recommendation: "実装済み foundation として扱う。以後は `excli-schema:*` を analysis proposal の前提に置く。"
---
...
obsidian excli-schema:infer format=json

# 特定フォルダ内のノートだけを対象にする
obsidian excli-schema:infer folder=notes format=json

# 指定プロパティを持たないノートを列挙する（バリデーション用途）
obsidian excli-schema:missing key=tags folder=notes format=tsv

# あるノートがスキーマに対してどう違反しているかを検査する
obsidian excli-schema:validate path=notes/project.md format=json
```
...
- `path=<path>` — 検査対象ノート（必須、反復可）
- `folder=<path>` — 参照 schema を作る母集団を絞る
- `tag=<tag>` — 参照 schema をタグで絞る
- `missing-threshold=<n>` — 欠落を issue にする coverage 閾値
- `fail-on=<low|high|none>` — batch 全体の失敗閾値
...
### `excli-schema:missing key=tags` （`format=tsv`）
...
### `excli-schema:validate path=notes/project.md` （`format=json`）
