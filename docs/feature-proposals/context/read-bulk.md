...
builtin_diff_assessment: "妥当。既存の read を複数回呼ぶ運用とは別に、AI 向けの token-budgeted bulk fetch という独立した価値がある。"
...
複数ノートを 1 回の command でまとめて取得するコマンド。明示 path 群、folder、tag などで対象を決め、本文と frontmatter をトークン予算つきで返す。
...
# tag で絞ってトークン上限内に収める
obsidian excli-read:bulk tag=暗号 max-tokens=8000 format=json
...
```markdown
<!-- read-bulk count=2 truncated=false -->
...
4. `max-tokens` を超えた時点で deterministic に打ち切る
5. `resolve-embeds` 指定時だけ `embed-resolve` の展開器を通す

token 数は文字数ベース近似でよい。大事なのは deterministic な truncation である。

## 責務分離

- `packages/core` — token budget、並び順、truncation、出力整形
