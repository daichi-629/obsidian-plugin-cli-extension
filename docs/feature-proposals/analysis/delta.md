...
vault の変更履歴を時刻ベースで問い合わせるコマンド。「いつ以降に何が変わったか」を構造化して返し、AI が vault の増分状態を把握できるようにする。concept 名は `delta` のまま維持しつつ、user-facing surface は `excli-diff:recent` を第一候補とする。
...
```bash
# 直近7日の変更を JSON で返す
obsidian excli-diff:recent days=7 format=json

# 特定日付以降の変更
obsidian excli-diff:recent since=2026-04-01 folder=Papers format=json

# AI の再読込用に path だけ返す
obsidian excli-diff:recent since=1h paths-only

# タグでフィルタ
obsidian excli-diff:recent days=7 tag=project format=json

# リンクグラフへの影響も含める
obsidian excli-diff:recent days=1 graph-impact format=json
```

## オプション設計

- `since=<duration|timestamp>` — 基準時刻。`1h` / `24h` / `7d` の相対表記、または ISO 8601 の絶対時刻
- `days=<n>` — `since` の shorthand。`days=7` は「直近7日」を意味する
...
`days=<n>` は conversation-start 用の ergonomic shorthand として扱う。feedback 上の主要ユースケースは「会話開始時に最近の変更を知る」ことであり、`excli-diff:recent days=7` をその標準形にする。
