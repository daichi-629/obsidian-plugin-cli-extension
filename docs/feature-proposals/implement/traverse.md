...
existing_overlap:
    - "excli-grep: ノート本文検索はできるが、link graph traversal や shortest path はできない"
...
recommendation: "実装済み foundation として扱う。graph 系 proposal は `excli-traverse:*` の上に積む。"
---

# Implemented foundation: traverse
...
```bash
# 2 ノート間の最短リンクパスを返す
obsidian excli-traverse:path from="PKM" to="機械学習" format=json

# あるノートから N ホップ以内に到達できるノート群
obsidian excli-traverse:reach from="プロジェクトX" depth=2 format=json

# 孤立したリンククラスターを検出する
obsidian excli-traverse:clusters min-size=3 format=json
```
...
- `from=<name|path>` — 起点ノート
- `to=<name|path>` — 終点ノート
- `direction=out|both` — 有向または無向 shortest path
- `folder=<path>` — 対象 subgraph を絞る
- `tag=<tag>` — 対象 subgraph をタグで絞る
- `format=json|tsv|text` — 出力形式
